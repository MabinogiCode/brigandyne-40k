...
export class BrigActor extends Actor {
  ...
  async rollWeaponAttack(item, options = {}) {
    ...
    const target = (Array.from(game.user.targets)[0] as any)?.actor;
  ...
  async _performTest(testData: any, { skipDialog = false, event }: { skipDialog?: boolean; event?: any } = {}) {
    const fastKey = event?.shiftKey || event?.ctrlKey;
    let dialogResult: Record<string, any> | null = { difficulty: 0, situational: 0, advantage: 0, disadvantage: 0, tactic: "" };
