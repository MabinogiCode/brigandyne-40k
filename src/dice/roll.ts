 ...
 */
export function degreeOf(total: number, target: number): string {
...
export function rerollCostFor(degree: string): number { return degree === "critFailure" ? 6 : 4; }
...
export class BrigTest {
  ...
  async toMessage({ rollMode }: { rollMode?: string } = {}) {
