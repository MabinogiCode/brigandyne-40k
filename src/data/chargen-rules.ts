 ...
 */
export function dropLowest(sums: number[]): { pool: number[]; dropped: number; droppedIndex: number } {
 ...
 */
export function pointBuyValid(points: Array<number | string>): boolean {
 ...
 */
export function applyPsyTransfer(totals: Record<string, number>, psyDraw: Record<string, number | string> = {}, cap: number | null = null): Record<string, number> {
  const out: Record<string, number> = { ...totals };
 ...
 */
export function atoutCount(cns: number): number {
 ...
 */
export interface AtoutEntry { name: string; kind: "specialty" | "talent"; specialtyType?: string; [key: string]: unknown }
export function selectableAtouts(entries: AtoutEntry[], { owned = [], psy = 0 }: { owned?: string[]; psy?: number } = {}): AtoutEntry[] {
 ...
 */
export function drawAtoutOptions<T>(pool: T[], rand: () => number = Math.random): T[] {
