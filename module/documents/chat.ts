...
async function onOppose(message) {
  ...
  if (!(actor?.system as any)?.characteristics) return ui.notifications?.warn(game.i18n.localize("BRIG.Warn.noOpposer"));
  const key = await pickCharacteristic(origTest.characteristic) as string | null;
  if (!key) return;
  const char = (actor.system as any).characteristics[key];
  ...
  ChatMessage.create({ speaker: message.speaker, content } as any);
}
...
async function onApplyDamage(message) {
  ...
  if (dmg.raw) {
    ...
    if ((item?.system as any)?.qualityValue) {
      const v = (item.system as any).qualityValue("perceArmure");
      if (typeof v === "number") ap = v;
      if ((item.system as any).hasQuality?.("ignoreArmures")) ignoreArmor = true;
      if ((item.system as any).hasQuality?.("armureMoitie")) halveArmor = true;
      if ((item.system as any).hasQuality?.("antiVehicule")) vehicleScale = true;
    }
  }
  ...
  for (const actor of targets) {
    const applied = await (actor as any).applyDamage(total, { ignoreArmor, ap, halveArmor, vehicleScale, minDamage: 1 });
    let prot: number | string = "—";
    if (!ignoreArmor) { let p = (actor.system as any).protection?.value ?? 0; if (halveArmor) p = Math.floor(p / 2); prot = Math.max(0, p - ap); }
...
async function onReroll(message, button) {
  ...
  if (!actor) return;
  if (!(actor as any).isOwner) return ui.notifications?.warn(game.i18n.localize("BRIG.Warn.notOwner"));
  const cost = rerollCostFor(flags.result.degree);
  const ok = await (actor as any).spendSangFroid(cost);
  if (!ok) return;
...
async function onRiposte(message, button) {
  ...
  const weapon = (target as any).items.find(i => i.type === "weapon" && i.system.isMelee && i.system.equipped)
    || (target as any).items.find(i => i.type === "weapon" && i.system.isMelee);
  const forBonus = (target.system as any).characteristics?.for?.bonus ?? 0;
  ...
  if ((weapon?.system as any)?.qualityValue) {
    const v = (weapon.system as any).qualityValue("perceArmure");
    if (typeof v === "number") ap = v;
    if ((weapon.system as any).hasQuality?.("ignoreArmures")) ignoreArmor = true;
  }
  const applied = await (attacker as any).applyDamage(total, { ap, ignoreArmor, minDamage: 1 });
  ...
  const content = await renderTemplate("systems/brigandyne-40k/templates/chat/damage-card.hbs", {
    ...
    rows: [{ name: (attacker as any).name, raw: total, applied, prot: ignoreArmor ? "—" : Math.max(0, ((attacker.system as any).protection?.value ?? 0) - ap) }],
    hasTargets: true
  });
  ChatMessage.create({ speaker: message.speaker, content } as any);
}
